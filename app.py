...
# TAB 2: MedRAS
with tab2:
    st.header("🔍 MedRAS 護理師智能評估卡")
    st.markdown("""
    根據 **FACE 圈 MedRAS 護備小卡** 指引，評估病患特質。
    只要 **其中一個符合**，即可建議家屬自費購買 **F&P 面罩** (臨床實證受壓極低，且兩側有設計 NG 槽，**可以不需減壓墊**)。
    """)
    
    col1, col2 = st.columns(2)
    with col1:
        st.subheader("📋 評估題目")
        medras_ng = st.checkbox("1. 臨床上是否有使用鼻胃管 (NG)？", value=False)
        medras_skin = st.checkbox("2. 臉部配戴面罩處是否有破皮，或屬於易破皮之高風險膚質？", value=False)
    
    with col2:
        st.subheader("💡 系統評估與決策建議")
        if medras_ng or medras_skin:
            st.markdown(f"""
            <div class='success-box'>
                <p class='info-tag'>🌟 決策：強烈建議使用自費 F&P 面罩！</p>
                <ul>
                    <li><b>原因：</b> 病患符合 MedRAS 評估指標（{'鼻胃管插管' if medras_ng else ''} {' ' if medras_ng and medras_skin else ''} {'臉部皮膚脆弱高風險' if medras_skin else ''}）。</li>
            ...
            <div class='alert-box'>
                ...
                <ul>
                    <li><b>評估結果：</b> 目前暫無鼻胃管且皮膚無破皮/易破皮風險。</li>
